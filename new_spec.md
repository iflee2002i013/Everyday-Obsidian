...
不要只保存在 `.obsidian/plugins/Everyday/data.json` 中。
...
```ts
const DEFAULT_SETTINGS: EverydaySettings = {
  diaryFolder: "Diary",
...
```ts
const VIEW_TYPE_MONTH_MEMORY = "Everyday-month-view";
```
...
```ts
function getDiaryFilePath(date: string, settings: EverydaySettings): string
```
...
class DiaryStorageService {
  constructor(app: App, settings: EverydaySettings)
...
.Everyday-header {}
.Everyday-day-row {}
.Everyday-day-row.is-empty {}
.Everyday-day-row.is-today {}
.Everyday-mood-pill {}
.Everyday-summary {}
```
